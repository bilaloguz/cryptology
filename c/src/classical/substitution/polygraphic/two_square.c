/**
 * @file two_square.c
 * @brief Two Square Cipher implementation
 */
...
int two_square_encrypt(const char *plaintext, const char *key1, const char *key2,
    ...
    if (!plaintext || !key1 || !key2 || !result || result_size == 0) {
        return -1;
    }
    ...
    for (size_t i = 0; i < prepared_len; i += 2) {
        if (i + 1 < prepared_len) {
            char digram[3];
            if (encrypt_digram(square1, square2, prepared_text[i], prepared_text[i + 1], digram) != 0) {
                return -1;
            }
            result[i] = digram[0];
            result[i + 1] = digram[1];
...
int two_square_decrypt(const char *ciphertext, const char *key1, const char *key2,
                       char *result, size_t result_size) {
    if (!ciphertext || !key1 || !key2 || !result || result_size == 0) {
        return -1;
    }
    ...
    for (size_t i = 0; i < prepared_len; i += 2) {
        if (i + 1 < prepared_len) {
            char digram[3];
            if (decrypt_digram(square1, square2, prepared_text[i], prepared_text[i + 1], digram) != 0) {
                return -1;
            }
            result[i] = digram[0];
            result[i + 1] = digram[1];
