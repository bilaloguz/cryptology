#include "cryptology/classical/transposition/scytale.h"
#include <string.h>
#include <stdlib.h>
#include <ctype.h>

static void clean_text(const char *input, char *output) {
    size_t i, j = 0;
    for (i = 0; input[i] != '\0'; i++) {
        if (isalpha(input[i])) {
...
int scytale_encrypt(const char *plaintext, int key, char *output, size_t output_size) {
    ...
    char *clean_input = malloc(strlen(plaintext) + 1);
    if (!clean_input) return -1;
    
    clean_text(plaintext, clean_input);
...
int scytale_decrypt(const char *ciphertext, int key, char *output, size_t output_size) {
    ...
    char *clean_input = malloc(strlen(ciphertext) + 1);
    if (!clean_input) return -1;
    
    clean_text(ciphertext, clean_input);
