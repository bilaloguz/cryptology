#!/usr/bin/env python3
"""
COMPREHENSIVE CIPHER TESTING - ALL CIPHERS, ALL ALPHABETS
...
# Test texts
TEST_TEXTS = {
    'english': "the quick brown fox is over the lazy dog",  # No 'j' for Playfair/Bifid/Trifid
...
def test_cipher(name, encrypt_func, decrypt_func, plaintext, *args, **kwargs):
    ...
    try:
        ...
        if not success and kwargs.get('show_details', False):
            print(f"         FAIL: '{orig_norm[:30]}...' vs '{decr_norm[:30]}...'")
...
try:
    ...
    plain = TEST_TEXTS['english']
    success, lower = test_cipher('Bifid', bifid.encrypt, bifid.decrypt, plain, 'secret', A.ENGLISH_ALPHABET, 'standard', None)
...
try:
    ...
    plain = TEST_TEXTS['english']
    success, lower = test_cipher('Trifid', trifid.encrypt, trifid.decrypt, plain, 'secret', A.ENGLISH_ALPHABET, 'standard', None)
